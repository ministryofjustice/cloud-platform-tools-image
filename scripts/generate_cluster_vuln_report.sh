...
jq -c '.items.[]' live_2_vuln.json | while read i; do
  ...
  jq --arg SOURCE_CODE "$SOURCE_CODE" --arg OBJ_UID "$OBJ_UID" '{items: [.items.[] | select(.metadata.uid | contains($OBJ_UID)) += {"cloud-platform.justice.gov.uk/source-code": $SOURCE_CODE }]}' updates_here.json > updated_source_code.json

  jq --arg OWNER "$OWNER" --arg OBJ_UID "$OBJ_UID" '{items: [.items.[] | select(.metadata.uid | contains($OBJ_UID)) += {"cloud-platform.justice.gov.uk/owner": $OWNER }]}' updated_source_code.json > updated_owner.json

  jq --arg TEAM_NAME "$TEAM_NAME" --arg OBJ_UID "$OBJ_UID" '{items: [.items.[] | select(.metadata.uid | contains($OBJ_UID)) += {"cloud-platform.justice.gov.uk/team-name": $TEAM_NAME }]}' updated_owner.json > updated_team_name.json
